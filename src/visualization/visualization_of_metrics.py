...
ddest = '/local_scratch/sharreve/paper/inhomog/model_results'
...
group_names = ['patient', 'patient_3T', 'volunteer', 'test']
...
for glcm_file_name in glcm_file_name_list:
    ...
    for k, v in RESULT_PATH.items():
        # print(k)
        temp_glcm_dict = helper_inhomog.collect_glcm(v, file_name=glcm_file_name, temp_dict=temp_metric_dict, key=k)
...
for data_type, model_dict in glcm_metric_dict.items():
    ...
    for model_name, metric_dict in model_dict.items():
        dataset_name = re.findall('(_patient_3T|_test|_patient|_volunteer)', model_name)[0]
        _ = glcm_aggregate_dict[data_type].setdefault(model_name, {})
        ...
        for metric_key in chosen_feature:
            ...
            _ = glcm_aggregate_dict[data_type][model_name].setdefault(metric_key, {})
            glcm_aggregate_dict[data_type][model_name][metric_key] = get_mean_std_str(metric_value)
            # If we are calculating stuff for prediction, we can immediatly  calculate the relative stuff with the input
            if data_type == 'pred':
                # Using a specific model's input.. because that makes life easier
                metric_value_input = np.array(glcm_metric_dict['input']['single_homog' + dataset_name][metric_key])
                metric_value_pred = np.array(metric_value)
                _ = glcm_aggregate_dict.setdefault('rel_pred', {})
                _ = glcm_aggregate_dict['rel_pred'].setdefault(model_name, {})
                _ = glcm_aggregate_dict['rel_pred'][model_name].setdefault(metric_key, {})
                glcm_aggregate_dict['rel_pred'][model_name][metric_key] = get_mean_std_str(
                    (metric_value_pred - metric_value_input) / metric_value_input)
            # Same thing for the target...
            if data_type == 'target':
                metric_value_input = np.array(glcm_metric_dict['input']['multi_homog' + dataset_name][metric_key])
                metric_value_target = np.array(metric_value)
                _ = glcm_aggregate_dict.setdefault('rel_target', {})
                _ = glcm_aggregate_dict['rel_target'].setdefault(model_name, {})
                _ = glcm_aggregate_dict['rel_target'][model_name].setdefault(metric_key, {})
                glcm_aggregate_dict['rel_target'][model_name][metric_key] = get_mean_std_str(
...
for i_group in group_names:
    for i_data_type in data_type_list:
        metric_data_frame = pd.DataFrame(glcm_aggregate_dict[i_data_type])
        group_columns = [x for x in metric_data_frame.columns if x.endswith(i_group)]
        if len(group_columns):
            print(f"\n{i_group}_{i_data_type}")
            print('\t\t', '\t'.join(chosen_feature))
            helper_inhomog.print_dataframe(metric_data_frame[group_columns].T)
...
for cov_file_name in cov_file_name_list:
    res_dict_coefv = {}
    for k, v in RESULT_PATH.items():
        res_dict_coefv = helper_inhomog.collect_a_npy_file(v, file_name=cov_file_name, temp_dict=res_dict_coefv, key=k)
    # relative COV stuff
    aggregate_coefv = harray.aggregate_dict_mean_value(res_dict_coefv, agg_dict={})
    coefv_df = pd.DataFrame([aggregate_coefv])
    coefv_df = coefv_df.applymap("{0:.2f}".format)
    for i_group in group_names:
        # group_columns = [x for x in RESULT_PATH.keys() if x.endswith(i_group)]
        group_columns = [x for x in coefv_df.columns if x.endswith(i_group)]
        if len(group_columns):
            print(f"\n{i_group} features - {cov_file_name}")
            helper_inhomog.print_dataframe(coefv_df[group_columns].T)
...
res_dict_rmse = {}
for k, v in RESULT_PATH.items():
    res_dict_rmse = helper_inhomog.collect_a_npy_file(v, 'rmse_values', temp_dict=res_dict_rmse, key=k)

rmse_aggregate_dict = {}
for k, v in res_dict_rmse.items():
    rmse_aggregate_dict[k] = get_mean_std_str(v)

rmse_df = pd.DataFrame([rmse_aggregate_dict])
group_columns = [x for x in RESULT_PATH.keys() if x.endswith('test')]
helper_inhomog.print_dataframe(rmse_df[group_columns].T)
...
res_dict_ssim = {}
for k, v in RESULT_PATH.items():
    res_dict_ssim = helper_inhomog.collect_a_npy_file(v, 'ssim_values', temp_dict=res_dict_ssim, key=k)
...
for k, v in res_dict_ssim.items():
    ...
    v[v>1] = 1
    v[v<0] = 0
    ssim_aggregate_dict[k] = get_mean_std_str(v)

ssim_df = pd.DataFrame([ssim_aggregate_dict])
group_columns = [x for x in RESULT_PATH.keys() if x.endswith('test')]
helper_inhomog.print_dataframe(ssim_df[group_columns].T)
...
res_dict_wass = {}
for k, v in RESULT_PATH.items():
    res_dict_wass = helper_inhomog.collect_a_npy_file(v, 'wasserstein_values', temp_dict=res_dict_wass, key=k)

wd_aggregate_dict = {}
for k, v in res_dict_wass.items():
    wd_aggregate_dict[k] = get_mean_std_str(v*100)

wd_df = pd.DataFrame([wd_aggregate_dict])
group_columns = [x for x in RESULT_PATH.keys() if x.endswith('test')]
helper_inhomog.print_dataframe(wd_df[group_columns].T)
...
for k, v in rel_iter_dict.items():
    ...
    sel_ax.fill_between(np.arange(len(v)), mean_value-std_1, mean_value+std_1, color=sel_color, alpha=0.1)
    sel_ax.set_xlabel('Number of iterations')
    sel_ax.set_ylabel('$\|\| y_i - y_{i+1}\|\|$')
    sel_fig.suptitle(name_converter[k_without_dataset])
