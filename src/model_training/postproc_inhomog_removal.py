...
class PostProcInhomogRemoval(PostProcBase):
    def __init__(self, executor_module, image_dir, mask_dir, dest_dir=None, target_dir=None, config_dict=None, config_path=None, config_name='config_run.json', **kwargs):
        ...
        # If set to True we limit the amount of maxed out values to occur.
        # This is deprecated
        # self.dynamic_tresholding = kwargs.get('dynamic_tresholding', False)
    ...
    def run_patches(self, x_img, x_mask):
        ...
        result_stiched = hmisc.correct_inf_nan(result_stiched)
        if self.target_type == 'both':
            second_result_stiched = harray.get_stiched(second_result_patches, x_img.shape[-2:],
                                                patch_shape=self.patch_shape, stride=self.stride)
            second_result_stiched = hmisc.correct_inf_nan(second_result_stiched)
    ...
    def postproc_loaded(self, result, slice_index=0):
        ...
        if self.target_type == 'biasfield':
            ...
            corrected_image = hmisc.correct_inf_nan(corrected_image) * self.loaded_mask[slice_index]
            ...
            if self.experimental_postproc_both == 0:
                ...
                calc_bias_field = hmisc.correct_inf_nan(calc_bias_field) * self.loaded_mask[slice_index]
            ...
            bias_field = hmisc.correct_inf_nan(bias_field) * self.loaded_mask[slice_index]
        ...
        corrected_multi_coil_img = hmisc.correct_inf_nan(corrected_multi_coil_img) * self.loaded_mask[slice_index]
